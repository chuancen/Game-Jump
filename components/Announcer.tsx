...
const Announcer: React.FC<AnnouncerProps> = ({ message }) => {
  ...
  return (
    <div className={`fixed z-[100] transition-all duration-700 ease-out transform 
      /* Desktop: Right-Side */
    ...
    `}>
      <div className={`bg-black/95 backdrop-blur-3xl px-4 py-4 md:px-6 md:py-5 rounded-2xl border-2 max-w-[200px] md:max-w-[260px] text-left flex flex-col ${colorClass}`}>
        <div className="flex items-center space-x-3 mb-2 md:mb-3">
          <div className="relative">
            <div className={`w-2 h-2 md:w-3 md:h-3 rounded-full ${avatarColor}`}></div>
            <div className={`absolute inset-0 w-2 h-2 md:w-3 md:h-3 rounded-full animate-ping ${avatarColor}`}></div>
          </div>
          <div className="text-[7px] md:text-[9px] uppercase tracking-[0.4em] font-bold font-orbitron opacity-70">Aurora Link</div>
        </div>
        <div className="relative">
          <p className="text-[10px] md:text-xs font-orbitron leading-relaxed italic font-medium">
            "{displayMessage.text}"
          </p>
        </div>
        <div className="mt-3 md:mt-4 w-full h-[2px] bg-white/10 rounded-full overflow-hidden">
